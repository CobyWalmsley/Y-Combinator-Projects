...
for key in stockdict:
    for x in range(300):
        ...
        if len(crazylist)!=300: #I need 300 points
            r=random.randint(1,1300) #choose from 1 to 1400 because it can't start any past 1400 if each point is 200 long
...
for j in datalist:
    if datalist.index(j)%500==0:#print every 500th value: progress meter
        print(datalist.index(j),"done")
    if datalist.index(j)%4==0: #every 6th value in datalist, append to test_data_original
        if datalist.index(j)%8==0:
            y=j.pop(0)                   
            p=len(j)
            ...
            o=j.pop(0)                   
            p=len(j)
            answer_array3=np.array([o])
            between_array3=np.array(j)
            question_array3 = between_array3.reshape((p,1))
            day_tuple3=(question_array3,answer_array3)
            validation_data.append(day_tuple3)
...
data_grabber()
net=Network([299,80,2],cost=CrossEntropyCost)
net.SGD(training_data, 15, 100, 1.5, lmbda = 1, evaluation_data=validation_data, monitor_evaluation_accuracy=True,)