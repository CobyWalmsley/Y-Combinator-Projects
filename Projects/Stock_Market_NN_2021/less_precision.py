...
#3/10
#58.90,84.31,137.18,38.70,61.22,19.09,25.23,15.42   424.96
#8.65,21.21,-6.24  35.10
...
data=yf.download('MU',start='2022-03-16', interval='1m',  end='2022-03-17.',progress=False)[['Close']]
data.head()
...
moving_average_list=x_axis_list[9:]
...
for n in range(4,len(string_data)): #check every point from number 10 to 384 in the day, to allow the first few minutes to build up.
    average=moving_average(string_data,n,8)
    if n>8:
    ...
    if n>8:
        ...
        if condition==1:
                ...
                #if buy_count>=2:
                    #if profit+current_gain<-(one_percent/2):
                        #emergency_cut=1
                ...
                if moving_average_slope_list2==[1,1,1,1,1,1]:
                    ...
                    if 0>factor>-4.4:
                        if slope_list==[0,1,1,0,0,0]:
                            sell_list.append(n)
                            if emergency_cut==0:       
                                plt.axvline(x=n,color='red')
                                profit=(profit+current_gain)
                                condition=0
                if moving_average_slope_list2==[0,0,1,1,1,1]:
                    if 0>factor>-3.65:
                        if slope_list==[0,0,0,0,0,0]:
                            sell_list.append(n)
                            if emergency_cut==0:       
