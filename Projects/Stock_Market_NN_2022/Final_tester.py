...
def multi_runthrough(filenames, net_file_name):
    ...
    for file in filenames:
        ...
        with open(file_path, "r") as f:
            ...
            f.close()
        day_list=eval(day_list)
        guess_list = [(np.argmax(net.feedforward(x)), np.argmax(y),z) for x,y,z in day_list]      
        for x,y,z in guess_list:
            guess_index+=1
            if x==2:
    ...
    accuracy = correct_buys/buy_count
    print(net_file_name,correct_buys, buy_count, accuracy)
    return(buy_guesses,correct_buy_list,correct_buys,buy_count,accuracy,day_list)

##We have horrible problems
##Not anymore babyyy ;)

def comparator(day_list,filenames,file_number):
    total_buy_guesses = []
    total_correct_buys = []
    overlaps0 = []
    overlaps1 = []
    ...
    file_path = f"C:/Users/cobyw/OneDrive/Documents/Machine_Learning/Stock_Prices/eval_accuracies.txt"
    with open(file_path, "w") as f:
        print("Creating accuracy file")
        f.write(f"{accuracy_dict}")
        f.close
    for i in total_buy_guesses:
        if total_buy_guesses.count(i)==(file_number): 
            overlaps0.append(i)
        if total_buy_guesses.count(i)==(file_number-1): 
            overlaps1.append(i)
        if total_buy_guesses.count(i) ==file_number-2:
            overlaps2.append(i)
        if total_buy_guesses.count(i) ==file_number-3:
            overlaps3.append(i)
        if total_buy_guesses.count(i) ==file_number-4:
            overlaps4.append(i)
        if total_buy_guesses.count(i) ==file_number-5:
            overlaps5.append(i)
        if total_buy_guesses.count(i) ==file_number-6:
            overlaps6.append(i)
    ...
    total_correct5 = 0
    for i in overlaps5:
        total_profit5 += full_list[i][2]
        if i in total_correct_buys:
            total_correct5 +=1
    total_profit6 = 0
    total_correct6 = 0
    for i in overlaps6:
        total_profit6 += full_list[i][2]
        if i in total_correct_buys:
            total_correct6 +=1

    print(f'Accuracy with {file_number} / {file_number} in agreement: {total_correct0} / {len(overlaps0)}  Profit: {total_profit0}')
    print(f'Accuracy with {file_number-1} / {file_number} in agreement: {total_correct1} / {len(overlaps1)} Profit: {total_profit1}')
    print(f'Accuracy with {file_number-2} / {file_number} in agreement: {total_correct2} / {len(overlaps2)} Profit: {total_profit2}')
    print(f'Accuracy with {file_number-3} / {file_number} in agreement: {total_correct3} / {len(overlaps3)} Profit: {total_profit3}')
    print(f'Accuracy with {file_number-4} / {file_number} in agreement: {total_correct4} / {len(overlaps4)} Profit: {total_profit4}')
    print(f'Accuracy with {file_number-5} / {file_number} in agreement: {total_correct5} / {len(overlaps5)} Profit: {total_profit5}')
    print(f'Accuracy with {file_number-6} / {file_number} in agreement: {total_correct6} / {len(overlaps6)} Profit: {total_profit6}')
...
def accuracy_collector(network_number,file_list):
    for n in range(network_number):
        comparator(file_list, [f"test_network{n}.txt"])

training_file_list = []
for n in range(27):
    training_file_list.append(f"test_network{n}.txt")
comparator([99],training_file_list,len(training_file_list))
    #check if there are any duplicates in the buy guesses, then check if they're right.
# 3 or more vote agreement was 358/547: 65.4% 
# 4 or more: 346/517: 66.9%
# 5: 306/469: 65.2%
# 6: 296/439: 67.4%
# 7: 296/427: 69.3%
# 8: 282/406: 69.5%
#9: 282/398: 70.9%
#10: 255/353: 72.2% (11 networks)
#11: 165/253: 65.2
#So its best to have one less than the total

#72.3%: 13 networks

#72.1%: 9 more accurate networks (65.31 avg. accuracy) the acuracy of whta they all agree on is 65.2%. Coincidence?
#69.6%: 4 most accurate networks (67.5 avg)



#81.8%: 4 most accurate networks but not allowing them all to agree
#90%!!! (90/100) 11 most accurate on eval data set equal to 10 in agreement
#Will now try all data at once with these conditions 660/1070: 61.6% 
# 75%: 34 networks, set to 33.
# 75%: 27 Networks, set to 26.
# 27 Networks set to 25: 83.3%